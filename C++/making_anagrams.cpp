...
int number_needed(string a, string b) {
    ...
    map<char,int> refMapA; //mapping the characters of string_B
    map<char,int> refMapB; //mapping the characters of string_B
    ...
    for(it = string_A.begin(); it != string_A.end(); it++){
        if(refMapB[it->first]){
            temp = (refMapB[it->first] - it->second);
            if(temp < 0) temp *= (-1);
            requiredNumber += temp;
            }
        else{
    ...
    for(it = string_B.begin(); it != string_B.end(); it++){
        if(!refMapA[it->first]){ //looking for characters that are not present in string A.
            requiredNumber += it->second;
            }
        }
